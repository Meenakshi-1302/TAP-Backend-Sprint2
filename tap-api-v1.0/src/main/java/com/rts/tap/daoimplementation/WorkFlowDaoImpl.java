...
public class WorkFlowDaoImpl implements WorkFlowDao {	
	...
	public WorkFlow findWorkFlowForRecruitmentProcessByMrf(MRF mrf) {
		...
		return (WorkFlow) eManager.createQuery(hql).setParameter("mrf", mrf).setParameter("type", MessageConstants.SET_WORKFLOW_TYPE_RECRUITMENT_PROCESS).getSingleResult();	
	}
	...
	public WorkFlow getWorkFlowByMrfId(Long mrfId){
		...
		return (WorkFlow) eManager.createQuery(hql).setParameter("mrfId", mrfId).setParameter("type", MessageConstants.SET_WORKFLOW_TYPE1).getSingleResult();
	}
